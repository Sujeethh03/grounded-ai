...
def main() -> None:
    parser = argparse.ArgumentParser(description="Hybrid search over indexed filings.")
    ...
    for i, hit in enumerate(hits, 1):
        print(f"\n[{i}] {hit.company_name} {hit.form_type} {hit.fiscal_year} — {hit.section}")
        print(f"    accession={hit.filing_accession}  rrf={hit.rrf_score}")
